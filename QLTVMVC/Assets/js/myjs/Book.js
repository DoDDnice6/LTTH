...
var uri = 'http://localhost:5399/api/BOOKs/';
    ...
    $.getJSON(uri)
        .done(function (data) {
            ...
            $.each(data, function (key, item) {
                $('#ListBook tbody').append('<tr id=Book' + item.ID + '><td>' + item.ID + '</td><td>' + item.Name + '</td><td>' + item.Author + '</td><td>' + item.PublishID+ '</td><td><button class="btn" data-toggle="modal" data-target="#myModal" data-id=' + item.ID + ' onclick="detail(' + item.ID + ')">Chi Tiết</buton><button class="btn" onclick="Delete(' + item.ID + ')">Xóa</buton></tr>')
            });
        });
});
